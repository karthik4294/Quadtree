...
double X0 =  3;	double Y0 = 12.732;
double X1 =  -9; double Y1 = -8;
double X2 =  15; double Y2 = -8;
...
{	
	if(c ==1)
	{
		...
		if(n->child[0] == NULL )
		{
			...
			double xt = n->pt[0].x-(n->width/2);double yt = n->pt[0].y-(n->height/2);
			//g = g+pow(((7*pow((xt-2),2)) - (6*sqrt(3)*(xt-2)*(yt-3)) + (13*pow((yt-3),2))- 16),2);
            g = g+pow((3*pow(xt-3,2) + 4*(xt-3)*(yt-2) - 3*pow(yt-2,2)- 8),2);
            //g = g+ pow(pow(xt-2,2) + pow(yt-3,2) - 4,2);
            //cout<<g<<"\n";
            h++;}
	...
	else if(c ==2)
	{
		if(n->child[0] == NULL )
		{
			...
			double xt =(n->pt[0].x+n->pt[1].x+n->pt[2].x)/3;double yt =(n->pt[0].y+n->pt[1].y+n->pt[2].y)/3;
			g = g+pow(3*pow(xt-3,2) + 4*(xt-3)*(yt-2) - 3*pow(yt-2,2)- 8,2);h++;}
		    //g = g+pow(((7*pow((xt-2),2)) - (6*sqrt(3)*(xt-2)*(yt-3)) + (13*pow((yt-3),2))- 16),2);h++;}
		}
	...
	else if(c == 3)
	{
		if(n->child[0] == NULL )
		{
			...
			double xt =(n->pt[0].x+n->pt[2].x)/2;double yt =(n->pt[0].y+n->pt[2].y)/2;
			g = g+pow(3*pow(xt-3,2) + 4*(xt-3)*(yt-2) - 3*pow(yt-2,2)- 8,2);h++;}
		    //g = g+pow(((7*pow((xt-2),2)) - (6*sqrt(3)*(xt-2)*(yt-3)) + (13*pow((yt-3),2))- 16),2);h++;}
		}
...
void qtree::node_Def(Node *n,Node *n_par,double theta,double R,double r,int c)
{
	switch(c)
	{
		...
		if(n_par == NULL)
		{
			n->pt[0].x = R*cos(((theta-inc)*Pi/180))+xcen; n->pt[0].y =R*sin(((theta-inc)*Pi/180))+ycen ;
			n->pt[1].x = R*cos((theta*Pi/180))+xcen; n->pt[1].y = R*sin((theta*Pi/180))+ycen;
			n->pt[2].x = r*cos((theta*Pi/180))+xcen; n->pt[2].y = r*sin((theta*Pi/180))+ycen;
			n->pt[3].x = r*cos(((theta-inc)*Pi/180))+xcen; n->pt[3].y = r*sin(((theta-inc)*Pi/180))+ycen;
			n->flag = WHITE;
			n->level = 0;			
		}
		break;
	...
	else if(c == 3)
	{
		...
		switch(k)
		{
			case 0:
				...
				n->pt[2].x = ((n_par->pt[0].x+n_par->pt[3].x)/2+(n_par->pt[1].x+n_par->pt[2].x)/2)/2;
				n->pt[2].y = ((n_par->pt[0].y+n_par->pt[3].y)/2+(n_par->pt[1].y+n_par->pt[2].y)/2)/2;
			...
			case 1:
				...
				n->pt[3].x = ((n_par->pt[0].x+n_par->pt[3].x)/2+(n_par->pt[1].x+n_par->pt[2].x)/2)/2;
				n->pt[3].y = ((n_par->pt[0].y+n_par->pt[3].y)/2+(n_par->pt[1].y+n_par->pt[2].y)/2)/2;			
			...
			case 2:
				...
				n->pt[0].x = ((n_par->pt[0].x+n_par->pt[3].x)/2+(n_par->pt[1].x+n_par->pt[2].x)/2)/2;;
				n->pt[0].y = ((n_par->pt[0].y+n_par->pt[3].y)/2+(n_par->pt[1].y+n_par->pt[2].y)/2)/2;;
			...
			case 3:
				...
				n->pt[1].x = ((n_par->pt[0].x+n_par->pt[3].x)/2+(n_par->pt[1].x+n_par->pt[2].x)/2)/2;;
				n->pt[1].y = ((n_par->pt[0].y+n_par->pt[3].y)/2+(n_par->pt[1].y+n_par->pt[2].y)/2)/2;;
...
//t = x;
	//t = pow(x-2,2)+pow(y-3,2)-4;//Circle 

	//t = 2*pow(x,2)+pow(y,2)-1;//Ellipse
	
	//t = 2*pow(x,2)-pow(y,2)-1;//Hyperbola
	
	//t = x*y-1;//Rectangular Hyperbola
	
	t = 3*pow(x-3,2) + 4*(x-3)*(y-2) - 3*pow(y-2,2) - 8;//Rotated Rectangular hyperbola
    
    //t = 7*pow((x-2),2) - (6*sqrt(3)*(x-2)*(y-3)) + 13*pow((y-3),2) - 16;
...
//    for(int kyo = 1;kyo<=10;kyo++)
  //       { 
	...
	for(double R = 5;R<=10;R=R+5)//Splitting initial circle into two concurrent circles
	{
...
cout<<"No: of function evaluations = "<<f<<"\n";	

cout<<"RMS value = "<<sqrt(g/h)<<"\n";
