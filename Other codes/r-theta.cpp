...
class qtree
{
	public:
		...
		void node_Def(Node *n,Node *n_par,double theta,double R,double r,FILE *p);		
...
void qtree::node_Def(Node *n,Node *n_par,double theta,double R,double r,FILE *p)
{
	if(n_par == NULL)
	{
...
void qtree::display(Node *n)
{
	...
	cout<<"\nx = "<<(n->pt[0].x+n->pt[2].x)/2;
	cout<<"\ty = "<<(n->pt[0].y+n->pt[2].y)/2<<"\n";
...
{  //	double i=(YMIN+((YMAX-YMIN)/box)),l = (YMIN+((YMAX-YMIN)/box));double j=(XMIN+((XMAX-XMIN)/box));
	...
	for(double R = 0.5;R<=2;R = R+0.5)
	{
		...
		{			
			...
			qt.node_Def(root,NULL,theta,R,r,p);
		
	                qt.buildtree(root,0);				
			
			//qt.create_q(root,q);
			
			//qt.nbr(root,root,m->choice);
