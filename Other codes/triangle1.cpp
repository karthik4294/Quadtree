...
void qtree::node_Def(Node *n,Node *n_par,double i,double j,int l)
{
	...
	n->pt[1].x = j-0.2; n->pt[1].y = -i-0.34641;
	n->pt[2].x = j+0.2; n->pt[2].y = -i-0.34641;
	n->flag = GREY;
	n->level = 0;			
	}
...
{  //	double i=(YMIN+((YMAX-YMIN)/box)),l = (YMIN+((YMAX-YMIN)/box));double j=(XMIN+((XMAX-XMIN)/box));
	...
	queue *q = new queue;
		...
		for(double i=-20;i<=20;i=i+0.34641)	
		{
			for(double j = -19.8;j<=19.8;j=j+0.4)
		...
		for(double i=19.65359;i>=-20;i=i-0.34641)	
		{
			for(double j = -20;j<=20;j=j+0.4)
